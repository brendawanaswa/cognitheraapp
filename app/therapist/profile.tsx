...
export default function TherapistProfile() {
  ...
  const [tab, setTab]                     = useState<"profile" | "availability" | "certs">("profile");
  ...
  const [bio, setBio]                     = useState("I'm a warm, compassionate therapist who believes healing happens when you feel truly heard. My approach blends cognitive-behavioral therapy with mindfulness practices.");
  const [experience, setExperience]       = useState("11");
  const [rate, setRate]                   = useState("8500");
  const [selectedSpecs, setSpecs]         = useState(["Anxiety & Panic", "Depression", "Trauma & PTSD"]);
  const [selectedSessions, setSessions]   = useState(["Video call", "Voice call"]);
      ...
      <View style={styles.profileHeader}>
        ...
        <View style={[
          ...
          <Text style={[
            ...
            {verified === "approved" ? "✅ Verified Therapist" : "🟡 Pending Verification"}
          </Text>
        </View>
        ...
        {(["profile", "availability", "certs"] as const).map(t => (
          ...
          >
            <Text style={[styles.tabText, tab === t && styles.tabTextActive]}>
              {t === "profile" ? "Profile" : t === "availability" ? "Availability" : "Certificates"}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {tab === "profile" && (
          <>
              ...
              <View style={styles.tags}>
                {SPECIALTIES.map(s => (
                  <TouchableOpacity
                    ...
                    style={[styles.tag, selectedSpecs.includes(s) && styles.tagActive]}
                    activeOpacity={editing ? 0.8 : 1}
                  >
                    <Text style={[styles.tagText, selectedSpecs.includes(s) && styles.tagTextActive]}>
                      {s}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              <View style={styles.tags}>
                {SESSION_TYPES.map(s => (
                  <TouchableOpacity
                    ...
                    style={[styles.tag, selectedSessions.includes(s) && styles.tagActive]}
                    activeOpacity={editing ? 0.8 : 1}
                  >
                    <Text style={[styles.tagText, selectedSessions.includes(s) && styles.tagTextActive]}>
                      {s}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              <View style={styles.tags}>
                {LANGUAGES.map(l => (
                  <TouchableOpacity
                    ...
                    style={[styles.tag, selectedLangs.includes(l) && styles.tagActive]}
                    activeOpacity={editing ? 0.8 : 1}
                  >
                    <Text style={[styles.tagText, selectedLangs.includes(l) && styles.tagTextActive]}>
                      {l}
                    </Text>
                  </TouchableOpacity>
                ))}
            ...
            {saved && (
              <Text style={styles.savedMsg}>✅ Profile updated successfully!</Text>
            )}
          </>
        )}
            ...
            <View style={styles.section}>
              ...
              {Object.entries(availability).map(([day, available]) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={[styles.dayLabel, available && styles.dayLabelActive]}>
                    {day}
                  </Text>
                  <View style={[styles.dayToggle, available && styles.dayToggleActive]}>
          ...
          <>
            <View style={styles.section}>
              ...
              <Text style={styles.sectionSub}>
                Keep your certificates up to date to maintain your verified status.
              </Text>

              <View style={[styles.verifyInfo, { backgroundColor: verified === "approved" ? `${C.green}18` : "#FEF3C7" }]}>
                <Text style={[styles.verifyInfoText, { color: verified === "approved" ? C.green : C.yellow }]}>
                  {verified === "approved"
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.certStatus}>
                        {uploaded ? "✅ Uploaded & Verified" : "⬆️ Tap to upload"}
                      </Text>
                    </View>
                    {uploaded
...
const styles = StyleSheet.create({
  ...
  avatar:               { width: 90, height: 90, borderRadius: 45, backgroundColor: C.primary, alignItems: "center", justifyContent: "center", borderWidth: 4, borderColor: C.primaryLight },
  avatarText:           { fontSize: 40, fontWeight: "800", color: C.white },
  avatarEditBtn:        { position: "absolute", bottom: 0, right: 0, width: 30, height: 30, borderRadius: 15, backgroundColor: C.purple, alignItems: "center", justifyContent: "center" },
  avatarEditIcon:       { fontSize: 14 },
  profileName:          { fontSize: 22, fontWeight: "800", color: C.text, marginBottom: 4 },
  profileTitle:         { fontSize: 14, color: C.primary, fontWeight: "600", marginBottom: 10 },
  verifiedBadge:        { borderRadius: 99, paddingHorizontal: 14, paddingVertical: 6, marginBottom: 16 },
  ...
  inputDisabled:        { backgroundColor: C.bg, borderColor: "transparent", color: C.text },
  inputMultiline:       { minHeight: 100 },
  tags:                 { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  tag:                  { backgroundColor: C.primaryLight, borderRadius: 99, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1.5, borderColor: "transparent" },
  ...
  saveBtn:              { backgroundColor: C.primary, borderRadius: 14, paddingVertical: 16, alignItems: "center", marginTop: 8, marginBottom: 12, shadowColor: C.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 8, elevation: 4 },
  saveBtnText:          { fontSize: 16, fontWeight: "700", color: C.white },
  savedMsg:             { textAlign: "center", fontSize: 14, color: C.green, fontWeight: "600", marginBottom: 16 },
  hoursRow:             { flexDirection: "row", gap: 12 },
  dayItem:              { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: C.border },
  dayItemActive:        { },
  dayLabel:             { fontSize: 15, fontWeight: "600", color: C.muted },
  dayLabelActive:       { color: C.text },
  dayToggle:            { backgroundColor: "#F3F4F6", borderRadius: 99, paddingHorizontal: 14, paddingVertical: 6 },
